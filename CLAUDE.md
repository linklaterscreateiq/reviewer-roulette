...
- `npm run bundle` — runs `tsdown` (the same bundler as `htmldiff`), producing the real artifact: a minified CommonJS bundle at `dist/roulette.js`. This is what `bin`/`main` point to and what consumers actually execute. `tsdown.config.ts` pins the output extension to `.js`, because tsdown would otherwise name a CJS build `roulette.cjs` and silently orphan `bin`/`main`. It does not type-check, so it is not a substitute for `npm run tsc`.
...
- `npm run smoke` — bundles, then runs `test/roulette.spec.ts` under Vitest: it spawns `dist/roulette.js` against stub Slack and GitLab servers on loopback and asserts on the comment it posts (author excluded, holiday reviewers excluded, one maintainer plus one other developer named), on the create/no-op/replace behaviour for an existing note, on it refusing to run at all when a required environment variable is missing, and on the run failing when GitLab rejects the write. No credentials or network access needed. Because it drives the bundle over real HTTP, it catches runtime breakage from dependency bumps that `tsc` passes clean.
- `npm test` — the same suite in watch mode. It does **not** rebuild, so re-run `npm run bundle` after touching `src/`, or the tests will keep exercising a stale artifact.

These are **end-to-end tests, not unit tests, and that is deliberate**: `src/roulette.ts` has no exports to import, and the point is to exercise the published bundle. Testing the TypeScript source instead would have passed while the shipped CLI was broken — which is the failure this repository has actually had, when an ESM-only dependency could not be `require()`d from the CommonJS bundle. Every case spawns the CLI in a child process against its own stub server on an ephemeral port, so they run concurrently.

- `npm run lint` — runs `biome ci --error-on-warnings .` over the whole repository: formatting, import order and lint rules. **Warnings fail the build**, so there is no standing backlog of them to learn to ignore; if one is genuinely wrong for this codebase, turn the rule off in `biome.json` and say why, rather than leaving it to accumulate. `npm run format` is the same check with `--write`, which fixes everything safely fixable. `biome.json` comes from `slidev-theme-createiq` and the two should stay in step rather than diverging; the only difference is `rules.preset` in place of the deprecated `rules.recommended`, which that repository still needs. Note the two-space indent it enforces is not what this repository used before it was adopted.
...
Verification before publishing is `npm run tsc` (type-check) plus `npm run smoke`. CI (`.github/workflows/build-node.yml`) runs all of them on the Node version in `mise.toml` (currently 24), installed there by `mise-action` so the version is declared in one place only.
...
The script reads everything from the environment. Every required value goes through `requiredEnv`, which exits 1 naming the variable. Do not reach for `process.env.X!` instead: it satisfies the compiler and does nothing at run time, which is how a missing `REVIEWER_BOT_USERNAME` used to leave the bot unable to recognise its own comment and post a duplicate on every pipeline run, exiting 0. Provided by the operator:
