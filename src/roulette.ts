...
const emojiNamesForStatus = (slackUser: Member) =>
  [
    ...
    ...(slackUser.profile?.status_emoji_display_info ?? []).flatMap(info => [info.emoji_name, info.display_alias]),
  ]
    .filter((name): name is string => !!name)
    .map(normaliseEmojiName)

// These were previously dereferenced with `!`, which asserts to the compiler but does nothing at
// run time: a missing REVIEWER_BOT_USERNAME left the bot unable to recognise its own comment and
// post a duplicate on every pipeline run, exiting 0.
const requiredEnv = (name: string): string => {
  const value = process.env[name]

  if (!value) {
    console.error(`${name} is not set. It is required; see the Runtime Configuration section of the README.`)
    process.exit(1)
  }
...
// Config
const config = {
  reviewerDataFile: requiredEnv('REVIEWER_CONFIG'), // '../../.reviewers.json'
  usernameOfBot: requiredEnv('REVIEWER_BOT_USERNAME'),
  slackToken: requiredEnv('REVIEWER_BOT_SLACK_TOKEN'),
}
// End Config
...
const getAllSlackUsers = async () => {
  ...
  if (!slackListUsers.members) {
    console.error('Slack returned no members from users.list, so no reviewer can be checked for being away.')
    process.exit(1)
  }
...
const runReviewRoulette = async () => {
  ...
  const startedPipelineUserId = Number.parseInt(requiredEnv('GITLAB_USER_ID'), 10)
...
`
  ...
  const gitlabAuthHeaders: Record<string, string> = { 'PRIVATE-TOKEN': requiredEnv('PROJECT_REVIEWER_BOT_PAT') }
  ...
  const notesResponse = await fetch(
    `${gitlabApiUrl}/api/v4/projects/${ciProjectId}/merge_requests/${ciMergeRequestIId}/notes?sort=asc`,
    {
  ...
  const replaceNote = !!previousNote?.body.includes('- [x] Give me two')
